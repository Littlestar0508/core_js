...
// 숫자 사이에 _ 가 있으면 단위를 표시하는 역할을 할 수 있다
...
let greaterThen = number[0] > number[2]; // 10 > 6
console.log(greaterThen); // true

// ~ 보다 크거나 작음 또는 같음 → boolean
let lessThenAndEqual = number[3] >= number[4]; // 101 >= 10,000
console.log(lessThenAndEqual); // false

let greaterThenAndEqual = number[3] <= number[4]; // 101 <= 10,000
console.log(greaterThenAndEqual); // true

// 동등(등급이나 정도가 같다) → boolean
let equal = number[0] == number[1]; // 10 == '10'
console.log(equal); // true

// 다름(같지 않음) → boolean
let unequal = number[0] != number[1]; // 10 != '10'
console.log(unequal); // false

// 동일(똑같다) → boolean
let same = number[0] === number[1]; // 10 === '10'
console.log(same); // false 변수형이 다르기 때문
